...
public class LineActivity extends Activity /* implements OnLongClickListener */{
	...
	/*
	 * @Override public boolean onLongClick(View v) { Log.i(TAG, "view: " +
	 * v.getId()); //获取最新的bean lineBean =
	 * dbAdapter.selectLineBeanById(lineBean.getId()); //跳转至坐标录入页面 Intent intent
	 * = new Intent(); intent.setClass(LineActivity.this,
	 * LocationActivity.class); int viewId = v.getId(); switch(viewId){ case
	 * R.id.btnChangeLanes: intent.putExtra(CustomConstant.LATITUDE,
	 * lineBean.getChangeLanesLat()); intent.putExtra(CustomConstant.LONGITUDE,
	 * lineBean.getChangeLanesLng()); intent.putExtra(CustomConstant.BEARING,
	 * lineBean.getChangeLanesBr()); break; case R.id.btnAheadDirectLine:
	 * intent.putExtra(CustomConstant.LATITUDE,
	 * lineBean.getAheadDirectLineLat());
	 * intent.putExtra(CustomConstant.LONGITUDE,
	 * lineBean.getAheadDirectLineLng());
	 * intent.putExtra(CustomConstant.BEARING, lineBean.getAheadDirectLineBr());
	 * break; case R.id.btnTurnLeft: intent.putExtra(CustomConstant.LATITUDE,
	 * lineBean.getTurnLeftLat()); intent.putExtra(CustomConstant.LONGITUDE,
	 * lineBean.getTurnLeftLng()); intent.putExtra(CustomConstant.BEARING,
	 * lineBean.getTurnLeftBr()); break; case R.id.btnTurnRight:
	 ...
	 * break; case R.id.btnSidewalk: intent.putExtra(CustomConstant.LATITUDE,
	 * lineBean.getSidewalkLat()); intent.putExtra(CustomConstant.LONGITUDE,
	 * lineBean.getSidewalkLng()); intent.putExtra(CustomConstant.BEARING,
	 * lineBean.getSidewalkBr()); break; case R.id.btnPassSchool:
	 ...
	 * break; case R.id.btnPassBusStation:
	 * intent.putExtra(CustomConstant.LATITUDE,
	 * lineBean.getPassBusStationLat());
	 * intent.putExtra(CustomConstant.LONGITUDE,
	 * lineBean.getPassBusStationLng()); intent.putExtra(CustomConstant.BEARING,
	 * lineBean.getPassBusStationBr()); break; case R.id.btnPassSidewalk:
	 ...
	 * break; case R.id.btnDirectLine: intent.putExtra(CustomConstant.LATITUDE,
	 * lineBean.getDirectLineLat()); intent.putExtra(CustomConstant.LONGITUDE,
	 * lineBean.getDirectLineLng()); intent.putExtra(CustomConstant.BEARING,
	 * lineBean.getDirectLineBr()); break; case R.id.btnEndDirectLine:
	 * intent.putExtra(CustomConstant.LATITUDE, lineBean.getEndDirectLineLat());
	 * intent.putExtra(CustomConstant.LONGITUDE,
	 * lineBean.getEndDirectLineLng()); intent.putExtra(CustomConstant.BEARING,
	 * lineBean.getEndDirectLineBr()); break; case R.id.btnOvertake:
	 ...
	 * intent.putExtra(CustomConstant.BEARING, lineBean.getOvertakeBr()); break;
	 * case R.id.btnTurn: intent.putExtra(CustomConstant.LATITUDE,
	 * lineBean.getTurnLat()); intent.putExtra(CustomConstant.LONGITUDE,
	 * lineBean.getTurnLng()); intent.putExtra(CustomConstant.BEARING,
	 * lineBean.getTurnBr()); break; case R.id.btnPullOver:
	 ...
	 * intent.putExtra(CustomConstant.BEARING, lineBean.getPullOverBr()); break;
	 * case R.id.btnPassing: intent.putExtra(CustomConstant.LATITUDE,
	 * lineBean.getPassingLat()); intent.putExtra(CustomConstant.LONGITUDE,
	 * lineBean.getPassingLng()); intent.putExtra(CustomConstant.BEARING,
	 * lineBean.getPassingBr()); break; } //哪个button的坐标
	 * intent.putExtra(CustomConstant.BUTTONID, viewId); //当前的经纬度
	 ...
	 * startActivityForResult(intent, ACTIVITY_ITEM_ADD); return true; }
	 */
	...
	private void fireHandler(Location location) {
		...
		List<LocationBean> locationList = dbAdapter
				.selectLocationByLineId(lineId);
		// 迭代保存的坐标点
		...
		/*
		 * Double changeLanesLat = lineBean.getChangeLanesLat(); Double
		 * changeLanesLng = lineBean.getChangeLanesLng(); Float changeLanesBr =
		 * lineBean.getChangeLanesBr();
		 * 
		 * if(null != changeLanesLat && null != changeLanesLng && 0 !=
		 * changeLanesLat && 0 != changeLanesLng){ if
		 * (Distance.GetDistance(curLongitude, curLatitude, changeLanesLng,
		 * changeLanesLat) < distance && Distance.getDiff(curBearing,
		 * changeLanesBr) < angleError) { //tvline.setText(R.string.toast06);
		 * playMusic(R.id.btnChangeLanes); openScoring(5); } }
		 * 
		 * Double aheadDirectLineLat = lineBean.getAheadDirectLineLat(); Double
		 * aheadDirectLineLng = lineBean.getAheadDirectLineLng(); Float
		 * aheadDirectLineBr = lineBean.getAheadDirectLineBr(); if(null !=
		 * aheadDirectLineLat && null != aheadDirectLineLng && 0 !=
		 * aheadDirectLineLat && 0 != aheadDirectLineLng){
		 * if(Distance.GetDistance(curLongitude, curLatitude,
		 * aheadDirectLineLng, aheadDirectLineLat) < distance &&
		 * Distance.getDiff(curBearing, aheadDirectLineBr) < angleError){
		 * //tvline.setText(R.string.toast07);
		 * playMusic(R.id.btnAheadDirectLine); openScoring(3); } }
		 * 
		 * Double turnLeftLat = lineBean.getTurnLat(); Double turnLeftLng =
		 * lineBean.getTurnLeftLng(); Float turnLeftBr =
		 * lineBean.getTurnLeftBr(); if(null != turnLeftLat && null !=
		 * turnLeftLng && 0 != turnLeftLat && 0 != turnLeftLng){
		 * if(Distance.GetDistance(curLongitude, curLatitude, turnLeftLng,
		 * turnLeftLat) < distance && Distance.getDiff(curBearing, turnLeftBr) <
		 * angleError){ //tvline.setText(R.string.toast08);
		 * playMusic(R.id.btnTurnLeft); openScoring(6); } } Double turnRightLat
		 * = lineBean.getTurnRightLat(); Double turnRightLng =
		 * lineBean.getTurnRightLng(); Float turnRightBr =
		 * lineBean.getTurnRightBr(); if(null != turnRightLat && null !=
		 * turnRightLng && turnRightLat != 0 && turnRightLng != 0){
		 * if(Distance.GetDistance(curLongitude, curLatitude, turnRightLng,
		 * turnRightLat) < distance && Distance.getDiff(curBearing, turnRightBr)
		 * < angleError){ //tvline.setText(R.string.toast09);
		 * playMusic(R.id.btnTurnRight); openScoring(6); } } Double sidewalkLat
		 * = lineBean.getSidewalkLat(); Double sidewalkLng =
		 * lineBean.getSidewalkLng(); Float sidewalkBr =
		 * lineBean.getSidewalkBr(); if(null != sidewalkLat && null !=
		 * sidewalkLng && 0 != sidewalkLat && 0 != sidewalkLng){
		 * if(Distance.GetDistance(curLongitude, curLatitude, sidewalkLng,
		 * sidewalkLat) < distance && Distance.getDiff(curBearing, sidewalkBr) <
		 * angleError){ //tvline.setText(R.string.toast10);
		 * playMusic(R.id.btnSidewalk); openScoring(7); } } Double passSchoolLat
		 * = lineBean.getPassSchoolLat(); Double passSchoolLng =
		 * lineBean.getPassSchoolLng(); Float passSchoolBr =
		 * lineBean.getPassSchoolBr(); if(null != passSchoolLat && null !=
		 * passSchoolLng && 0 != passSchoolLat && 0 != passSchoolLng){
		 * if(Distance.GetDistance(curLongitude, curLatitude, passSchoolLng,
		 * passSchoolLat) < distance && Distance.getDiff(curBearing,
		 * passSchoolBr) < angleError){ //tvline.setText(R.string.toast11);
		 * playMusic(R.id.btnPassSchool); openScoring(8); } } Double
		 * passBusStationLat = lineBean.getPassBusStationLat(); Double
		 * passBusStationLng = lineBean.getPassBusStationLng(); Float
		 * passBusStationBr = lineBean.getPassBusStationBr(); if(null !=
		 * passBusStationLat && null != passBusStationLng && 0 !=
		 * passBusStationLat && 0 != passBusStationLng){
		 * if(Distance.GetDistance(curLongitude, curLatitude, passBusStationLng,
		 * passBusStationLat) < distance && Distance.getDiff(curBearing,
		 * passBusStationBr) < angleError){ //tvline.setText(R.string.toast12);
		 * playMusic(R.id.btnPassBusStation); openScoring(9); } } Double
		 * passSidewalkLat = lineBean.getPassSidewalkLat(); Double
		 * passSidewalkLng = lineBean.getPassSidewalkLng(); Float passSidewalkBr
		 * = lineBean.getPassSidewalkBr(); if(null != passSidewalkLat && null !=
		 * passSidewalkLng && 0 != passSidewalkLat && 0 != passSidewalkLng){
		 * if(Distance.GetDistance(curLongitude, curLatitude, passSidewalkLng,
		 * passSidewalkLat) < distance && Distance.getDiff(curBearing,
		 * passSidewalkBr) < angleError){ //tvline.setText(R.string.toast13);
		 * playMusic(R.id.btnPassSidewalk); openScoring(7); } } Double
		 * directLineLat = lineBean.getDirectLineLat(); Double directLineLng =
		 * lineBean.getDirectLineLng(); Float directLineBr =
		 * lineBean.getDirectLineBr(); if(null != directLineLat && null !=
		 * directLineLng && 0 != directLineLat && 0 != directLineLng){
		 * if(Distance.GetDistance(curLongitude, curLatitude, directLineLng,
		 * directLineLat) < distance && Distance.getDiff(curBearing,
		 * directLineBr) < angleError){ //tvline.setText(R.string.toast14);
		 * playMusic(R.id.btnDirectLine); openScoring(3); } } Double
		 * endDirectLineLat = lineBean.getEndDirectLineLat(); Double
		 * endDirectLineLng = lineBean.getEndDirectLineLng(); Float
		 * endDirectLineBr = lineBean.getEndDirectLineBr(); if(null !=
		 * endDirectLineLat && null != endDirectLineLng && 0 != endDirectLineLat
		 * && 0 != endDirectLineLng){ if(Distance.GetDistance(curLongitude,
		 * curLatitude, endDirectLineLng, endDirectLineLat) < distance &&
		 * Distance.getDiff(curBearing, endDirectLineBr) < angleError){
		 * //tvline.setText(R.string.toast15); playMusic(R.id.btnEndDirectLine);
		 * openScoring(3); } } Double overtakeLat = lineBean.getOvertakeLat();
		 * Double overtakeLng = lineBean.getOvertakeLng(); Float overtakeBr =
		 * lineBean.getOvertakeBr(); if(null != overtakeLat && null !=
		 * overtakeLng && 0 != overtakeLat && 0 != overtakeLng){
		 * if(Distance.GetDistance(curLongitude, curLatitude, overtakeLng,
		 * overtakeLat) < distance && Distance.getDiff(curBearing, overtakeBr) <
		 * angleError){ //tvline.setText(R.string.toast16);
		 * playMusic(R.id.btnOvertake); openScoring(11); } } Double turnLat =
		 * lineBean.getTurnLat(); Double turnLng = lineBean.getTurnLng(); Float
		 * turnBr = lineBean.getTurnBr(); if(null != turnLat && null != turnLng
		 * && 0 != turnLat && 0 != turnLng){
		 * if(Distance.GetDistance(curLongitude, curLatitude, turnLng, turnLat)
		 * < distance && Distance.getDiff(curBearing, turnBr) < angleError){
		 * //tvline.setText(R.string.toast17); playMusic(R.id.btnTurn);
		 * openScoring(12); } }
		 * 
		 * Double pullOverLat = lineBean.getPullOverLat(); Double pullOverLng =
		 * lineBean.getPullOverLng(); Float pullOverBr =
		 * lineBean.getPullOverBr(); if(null != pullOverLat && null !=
		 * pullOverLng && 0 != pullOverLat && 0 != pullOverLng){
		 * if(Distance.GetDistance(curLongitude, curLatitude, pullOverLng,
		 * pullOverLat) < distance && Distance.getDiff(curBearing, pullOverBr) <
		 * angleError){ //tvline.setText(R.string.toast18);
		 * playMusic(R.id.btnPullOver); openScoring(13); } }
		 * 
		 * Double passingLat = lineBean.getPassingLat(); Double passingLng =
		 * lineBean.getPassingLng(); Float passingBr = lineBean.getPassingBr();
		 * if(null != passingLat && null != passingLng && 0 != passingLat && 0
		 * != passingLng){ if(Distance.GetDistance(curLongitude, curLatitude,
		 * passingLng, passingLat) < distance && Distance.getDiff(curBearing,
		 * passingBr) < angleError){ //tvline.setText(R.string.toast19);
		 * playMusic(R.id.btnPassing); openScoring(10); } }
		 */
	}
